...
implements ÁrbolBinarioOrdenado<C>{
    ...
    protected NodoBOLigado<C> findNode(C elem){
        if(!contains(elem)){
            return null;
        }
    ...
    protected void intercambia(NodoBinario<C> nodo1, NodoBinario<C> nodo2){
        C temp = nodo1.getElemento();
        nodo1.setElemento(nodo2.getElemento());
        nodo2.setElemento(temp);
    }

    protected NodoBOLigado<C> remueveNodo(C elem){
        ...
        if(gradoNodo == 0){ // Si es hoja se elimina sin problemas.
            if(aElim == raiz){
                ...
                NodoBinario<C> padreElim = aElim.getPadre();
            ...
            NodoBOLigado<C> hijoDer = (NodoBOLigado<C>) aElim.getHijoD();
            NodoBOLigado<C> minDer = hijoDer.findMin(); // El nodo más pequeño del subárbol derecho.
