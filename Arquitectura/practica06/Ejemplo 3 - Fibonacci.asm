...
main:	# Preambulo
	...
	sw	$ra, 16($sp)	# 2 Guardar $ra
	sw	$fp, 20($sp)	# 3 Guardar $fp
...
fibo:	# Preambulo
	...
	sw	$t0, 24($sp)	# 2 Guardar el valor de fibo(n-1) en la pila
	jal	fibo		# 3 Brinca subrutina (al final se guarda el resultado en v0)
	# Retorno
	lw	$t0, 24($sp)	# 1 Restaurar $t0
	add	$t0, $t0, $v0   # fibi(n-1) + fibo(n-2)
	j	end		# Fin then
