...
public class Principal{
    ...
    */
        ...
        if(g.dirigida){ //Si es dirigida
            for(Vertice v : g.vertices.values()){
                for(Vertice u : v.ady){
                    ...
                    if(g.tieneFlujo){
                        graph.getEdge(nombreArista).addAttribute("ui.label", g.getFlujo(v,u)+","+g.getPeso(v,u));
                    }else{
                        graph.getEdge(nombreArista).addAttribute("ui.label", g.getPeso(v,u));
                    }
                }
            }
    ...
    public static void main(String[] args){
        ...
        Grafica g2 = Algoritmo.flujoMaximo(g1);
