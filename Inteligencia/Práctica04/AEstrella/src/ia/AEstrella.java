...
public class AEstrella extends PApplet {
    ...
    class Mosaico{
        ...
        void calculaHeuristica(Mosaico meta){
            ...
            */
            int d1 = Math.abs(meta.renglon - this.renglon);
            int d2 = Math.abs(meta.columna - this.columna);
            hn = (d1+d2) * 10;
        }
        ...
        */
        ...
        //Función implementada por mí que verifica si un mosaico es vecino de otro
        boolean esVecino(Mosaico v){
            for(int i=-1;i<2;i++){
                for(int j=-1;j<2;j++){
                    if(columna+i>=0 && renglon+j>=0){
                        if(mapa.mundo[renglon+j][columna+i].equals(v)){
                            return true;
                        }
                    }
                }
    ...
    class NodoBusqueda implements Comparable<NodoBusqueda> {
        ...
        int getFn() {
            return estado.gn + estado.hn; //Modificada por mí
        }
        ...
        LinkedList<NodoBusqueda> getSucesores() {
            ...
            for(Accion a : Accion.values()) {
                ...
                if(sucesor != null) {
                    ...
                    nodoSucesor.estado.gn = nodoSucesor.gn; //Línea agregada por mí
    ...
    class Algoritmo {
        ...
        void inicializa(Mosaico estadoInicial, Mosaico estadoFinal) {
            ...
            nodoPrevio.gn = 0;
            nodoPrevio.estado.hn = 0;
            nodoPrevio.estado.gn = 0;
        ...
        void expandeNodoSiguiente() {
            ...
            if(!resuelto){
               ...
               while(!nodoPrevio.estado.esVecino(nodoActual.estado)){
                   nodoActual.estado.situacion = Situacion.SIN_VISITAR;
                   nodoActual = listaAbierta.poll();
               }
               ...
               if(nodoActual.estado == estadoFinal){
                   resuelto = true;
                   LinkedList<NodoBusqueda> sucs = nodoActual.getSucesores();
                   nodoActual.estado.situacion = Situacion.EN_SOLUCION;
                   for(NodoBusqueda N:sucs){
                       N.estado.situacion = Situacion.EN_SOLUCION;
                   }
               }else{
                   LinkedList<NodoBusqueda> sucs = nodoActual.getSucesores();
                   for(NodoBusqueda N:sucs){
                       if(N.estado.situacion != Situacion.EN_LISTA_ABIERTA && N.estado.situacion != Situacion.EN_LISTA_CERRADA){
