...
(define-type RCFWBAEL
   ...
   [funS (params (listof param?)) (result Type?) (body RCFWBAEL?)]
...
(define-type BindingS
   [bindingS (name symbol?) (tipo Type?) (value RCFWBAEL?)])
...
;; Para representar los tipos de datos aceptados.
(define-type Type
   [a] ;; Para tipos polimórficos
   [tnumber]
   [tboolean]
   [tlistof (tipo Type?)]
   [tarrow (arg Type?) (result Type?)])

;; Para representar los ambientes de tipos.
(define-type Type-Env
   [t-mtSub]
   [t-aSub (name symbol?) (type Type?) (tenv Type-Env?)])
