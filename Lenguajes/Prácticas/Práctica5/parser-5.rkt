...
(define (parse sexp)
   (match sexp
     ...
     [(? symbol?) (idS sexp)]
     [(? number?) (numS sexp)]
     [(list 'if test truth falsity) (ifS (parse test) (parse truth) (parse falsity))]
     [(cons 'cond resto) (condS (mpc resto))]
...
(define (desugar sexps)
   (match sexps
     ...
     [(ifS test truth falsity) (iF (desugar test) (desugar truth) (desugar falsity))]
     [(condS cola) (dscond cola)]
...
;A partir de aquí ponemos las funciones auxiliares

;;Función que quita el azúcar a cond
(define (dscond l)
  (if (empty? l)
      false
      (type-case Condition (first l)
        [condition (e t) (iF (desugar e) (desugar t) (dscond (rest l)))]
...
(define (mpc l)
  (cond
    [(empty? l) l]
    [(and (symbol? (first (first l))) (symbol=? (first (first l)) 'else)) (cons (else-cond (parse (second (first l)))) empty)]
    [else (cons (condition (parse (first (first l))) (parse (second (first l)))) (mpc (rest l)))]))
